...
joined as (
    ...
    select
        ...
        row_number() over (
            ...
            order by t.touch_ts asc nulls last
        ) as rn
...
select
    ...
    purchase_ts,
    purchase_value,
    coalesce(traffic_source_source, '(direct)') as first_source,
    coalesce(traffic_source_medium, '(none)') as first_medium,
    coalesce(traffic_source_campaign, '(none)') as first_campaign
from joined
where rn = 1